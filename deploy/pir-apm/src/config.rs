...
impl Config {
    ...
    pub fn from_map(vars: &HashMap<String, String>) -> Result<Self> {
        ...
        let defaults = Schema::memo_default();
...
mod tests {
    ...
    #[test]
    fn defaults_describe_the_memo_coordinator() {
        let config = Config::from_map(&HashMap::new()).unwrap();
        assert_eq!(config.scrape_url, "http://127.0.0.1:8080");
        assert_eq!(config.health_path, "/memo/health");
        assert_eq!(config.ready_path, "/ready");
        assert_eq!(config.metrics_path, "/metrics");
        assert_eq!(config.schema, Schema::memo_default());
        assert_eq!(config.title, "Memo PIR APM");
