...
# mktemp creates the file 0600; the webhook value is never echoed anywhere else.
{
  ...
  # Seed list only: the sidecar also picks up any endpoint label the
  # coordinator exports (per-table routes), so new tables need no change here.
  echo "PIR_APM_ENDPOINTS=health,metadata,generation,params,public_params,query"
  echo "PIR_APM_INFORMATIONAL_ENDPOINTS=health"
  echo "PIR_APM_PROCESSING_ENDPOINTS=query,witness_query,nf_cold_query,nf_warm_query"
  echo "PIR_APM_LATENCY_P99_SECONDS=1.0"
  echo "PIR_APM_LATENCY_P99_OVERRIDES=query=5.0,witness_query=5.0,nf_cold_query=5.0,nf_warm_query=5.0,public_params=2.0,witness_public_params=2.0,nf_cold_public_params=2.0,nf_warm_public_params=2.0"
