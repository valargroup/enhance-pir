...
//! This test is the rehearsal for `docs/memo-pir-deploy.md`, "Adding the
...
use memo_pir::coordinator::{CoordinatorState, TableSetup, WorkerTarget};
use memo_pir::store::RecordJournal;
use memo_pir::types::{
    worker_index_for_shard, ActionRecord, ActionRecordParts, DatabaseId, ACTION_LAYOUT,
...
fn placement_rule_for_the_migration_is_as_documented() {
    ...
    for shard in 0..10 {
        assert_eq!(worker_index_for_shard(shard, &["w1"]), Some(0));
    }
    ...
    for shard in 0..SHARDS_PER_WORKER {
        assert_eq!(worker_index_for_shard(shard, &["w1", "w2"]), Some(0));
    }
    for shard in SHARDS_PER_WORKER..2 * SHARDS_PER_WORKER {
        assert_eq!(worker_index_for_shard(shard, &["w1", "w2"]), Some(1));
    }
    assert_eq!(
        worker_index_for_shard(2 * SHARDS_PER_WORKER, &["w1", "w2"]),
        None
    );
    // Every append after the first moves nothing.
    for shard in 0..2 * SHARDS_PER_WORKER {
        assert_eq!(
            worker_index_for_shard(shard, &["w1", "w2"]),
            worker_index_for_shard(shard, &["w1", "w2", "w3"])
        );
    }
}
...
async fn second_worker_takes_the_upper_shards_without_changing_answers() {
    ...
    let manifest_before = before.manifest().expect("manifest").tables[&DatabaseId::Action].clone();
    ...
    let answers_before: Vec<ActionRecord> = {
    ...
    let manifest_after = after.manifest().expect("manifest").tables[&DatabaseId::Action].clone();
    ...
    let refused = worker_1
        .evaluate_local(
            DatabaseId::Action,
            memo_pir::wire::EvaluateRequest {
                generation,
                shards: (0..3)
                    .map(|shard_id| memo_pir::wire::ShardQuery {
                        shard_id,
                        coefficients: vec![0; ACTION_LAYOUT.shard_rows],
                    })
                    .collect(),
            },
        )
