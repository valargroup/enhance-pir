use crate::types::{
    setup_seed_bytes, EnhanceGeneration, EnhanceRecord, ENHANCE_SETUP_SEED, ITEM_SIZE_BITS,
    NETWORK, POOL, PROTOCOL_REVISION, RECORDS_PER_ROW, RECORD_BYTES, ROW_BYTES, SCHEMA_VERSION,
    SHARD_ROWS,
};
...
impl EnhancePirClient {
    pub async fn connect(base_url: &str) -> Result<Self, ClientError> {
        ...
        let ypir: YpirSchemeParams = serde_json::from_slice(
            &read_limited(
                http.get(format!("{base_url}/v1/enhance/params"))
                    .send()
                    .await?,
                64 * 1024,
            )
        ...
        let public_params = read_limited(
            http.get(format!("{base_url}/v1/enhance/public-params"))
                .send()
                .await?,
            16 * 1024 * 1024,
        )
